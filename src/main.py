...
from train import train_epoch, evaluate, predict_corrections
...
def main():
    ...
    train_files = [
        Path("/Users/nurlanmalikov7294/Documents/naic/spellchecker/data/pair_split_v2/train_part1.json")
    ]

    val_files = [
        Path("/Users/nurlanmalikov7294/Documents/naic/spellchecker/data/pair_split_v2/test.json")
    ...
    # Enable high matmul precision for MPS/CUDA
    ...
    # Use best model checkpoint by default
    checkpoint_path = Path(CHECKPOINT_DIR) / "epoch_1.pt"
    ...
    if checkpoint_path is not None and os.path.exists(checkpoint_path):
        ...
        if metadata.get('missing_keys') or metadata.get('unexpected_keys'):
            print("Checkpoint loaded with mismatched keys:")
            if metadata.get('missing_keys'):
                print(f"  Missing keys: {len(metadata['missing_keys'])}")
            if metadata.get('unexpected_keys'):
                print(f"  Unexpected keys: {len(metadata['unexpected_keys'])}")
