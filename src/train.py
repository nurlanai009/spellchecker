...
DEFAULT_TRAIN_PATH = Path("/Users/nurlanmalikov7294/Documents/naic/spellchecker/data/train")
DEFAULT_TEST_PATH = Path("/Users/nurlanmalikov7294/Documents/naic/spellchecker/data/test")
DEFAULT_VOCAB_PATH = Path("/Users/nurlanmalikov7294/Documents/naic/spellchecker/data/pair/vocab.json")
DEFAULT_CHECKPOINT_DIR = Path("/Users/nurlanmalikov7294/Documents/naic/spellchecker/checkpoints")


def _enable_gpu_optimizations(device: torch.device) -> None:
    if device.type == "cuda":
...
def _get_autocast_context(device: torch.device, fp16: bool):
    if not fp16:
        return nullcontext()
    if device.type == "cuda":
        return autocast()
...
def _resolve_data_paths(path: Path) -> List[Path]:
    if path.is_dir():
        return sorted([p for p in path.iterdir() if p.is_file() and p.suffix in {".json", ".jsonl"}])
    return [path]
...
def load_default_dataloaders(
    ...
    truncate_train: Optional[float] = None,
    truncate_test: Optional[float] = None,
):
    train_paths = _resolve_data_paths(DEFAULT_TRAIN_PATH)
    test_paths = _resolve_data_paths(DEFAULT_TEST_PATH)
    return get_dataloaders(
        ...
        vocab_save_path=str(DEFAULT_VOCAB_PATH),
...
def train_epoch(
    ...
    model.train()
    _enable_gpu_optimizations(device)
    epoch_loss = 0
    step = 0
    ...
    for batch in progress_bar:
        ...
        # Get data
        clean_ids = batch["clean_ids"].to(device, non_blocking=True)
        noisy_ids = batch["noisy_ids"].to(device, non_blocking=True)
        ...
        amp_ctx = _get_autocast_context(device, fp16)
        with amp_ctx:
            # Forward pass
            output, _ = model(
                noisy_ids,
...
def predict_corrections(
    ...
    # Encode the texts
    raw_encoded_texts = [tokenizer.encode(text) for text in texts]
    encoded_lengths = torch.tensor([len(ids) for ids in raw_encoded_texts], dtype=torch.long)
    encoded_texts = [tokenizer.pad_sequence(ids) for ids in raw_encoded_texts]
    input_tensor = torch.tensor(encoded_texts, dtype=torch.long).to(device, non_blocking=True)
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(input_tensor), batch_size):
            ...
            batch_lengths = encoded_lengths[i:i+batch_size]
...
def compute_masked_loss(output, target, criterion, pad_token_id, eos_token_id):
    ...
    target = target[:, 1:].contiguous()  # [B, T-1]

    # Mask for padding
    pad_mask = (target != pad_token_id)  # [B, T-1]

    # Mask out tokens after the *first* EOS in each sequence (keep up to first EOS)
    is_eos = (target == eos_token_id).int()          # [B, T-1]
    # Flag exactly the first EOS in each sequence
    is_first_eos = (is_eos.bool()) & (is_eos.cumsum(dim=1) == 1)
    first_eos_cum = is_first_eos.cumsum(dim=1)       # stays 0 before first EOS, 1 at/after
    # Positions strictly after first EOS
    after_first_eos = (first_eos_cum > 0) & (~is_first_eos)
    eos_mask = ~after_first_eos                      # keep before and at first EOS

    # Combine masks
    mask = pad_mask & eos_mask                       # [B, T-1]

    # Apply mask
    output = output[mask].view(-1, output.size(-1))  # [N, V]
    target = target[mask].view(-1)                   # [N]
...
def train_epoch(
    ...
    for batch in progress_bar:
        ...
        clean_ids = batch["clean_ids"].to(device, non_blocking=True)
        noisy_ids = batch["noisy_ids"].to(device, non_blocking=True)
        noisy_lengths = batch.get("noisy_lengths")  # stays on CPU for pack_padded_sequence
        clean_texts = batch["clean_texts"]
        ...
        if fp16:
            with autocast():
                # Forward pass (now passing lengths so encoder can use packing)
...
def evaluate(
    ...
    with torch.no_grad():
        for batch in tqdm(iterator, desc="Evaluating"):
            ...
            clean_ids = batch["clean_ids"].to(device, non_blocking=True)
            noisy_ids = batch["noisy_ids"].to(device, non_blocking=True)
            noisy_lengths = batch.get("noisy_lengths")  # CPU lengths
            clean_texts = batch["clean_texts"]
