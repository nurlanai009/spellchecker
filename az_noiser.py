...
from noiser_src.vocab import CASE, ENG, RULE_BASED, ADJACENT, WEIGHTED
...
class AzerbaijaniTextNoiser:
    ...
    def _apply_weighted_transformation(self, text: str) -> str:
        ...
        Do not touch tokens that are digits only. For mixed tokens, only
        letter characters can change (digits remain as they are).
        """
        noisy = []
        for word in text.split():
    ...
    def _apply_swap_transformation(self, text: str) -> str:
        ...
        Do not touch tokens that are digits only. In mixed tokens, do not
        swap positions of digits, only swap adjacent non-digit pairs.
        """
        noisy = []
        for word in text.split():
